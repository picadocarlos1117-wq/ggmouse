...
public abstract class OpenSSLAeadCipher extends OpenSSLCipher {
    ...
    private void checkInitialization() {
        if (this.mustInitialize) {
            f40.m2719o("Cannot re-use same key and IV for multiple encryptions");
        }
    }
    ...
    public void checkSupportedTagLength(int i) throws InvalidAlgorithmParameterException {
        if (i % 8 != 0) {
            throw new InvalidAlgorithmParameterException(jd0.m3609g(i, "Tag length must be a multiple of 8; was "));
        }
    }
    ...
    public int doFinalInternal(byte[] bArr, int i, int i2) throws BadPaddingException {
        ...
        try {
            ...
            long j = this.evpAead;
            int iEVP_AEAD_CTX_seal = zIsEncrypting ? NativeCrypto.EVP_AEAD_CTX_seal(j, this.encodedKey, this.tagLengthInBytes, bArr, i, this.f8444iv, this.buf, 0, this.bufCount, this.aad) : NativeCrypto.EVP_AEAD_CTX_open(j, this.encodedKey, this.tagLengthInBytes, bArr, i, this.f8444iv, this.buf, 0, this.bufCount, this.aad);
            if (isEncrypting()) {
    ...
    public int engineDoFinal(ByteBuffer byteBuffer, ByteBuffer byteBuffer2) throws BadPaddingException, ShortBufferWithoutStackTraceException {
        if (byteBuffer == null || byteBuffer2 == null) {
            l41.m4051t("Null ByteBuffer Error");
            return 0;
        }
        ...
        if (byteBuffer2.isReadOnly()) {
            f40.m2713i("Cannot write to Read Only ByteBuffer");
            return 0;
        }
        if (this.bufCount != 0) {
    ...
    public void engineInitInternal(byte[] bArr, AlgorithmParameterSpec algorithmParameterSpec, SecureRandom secureRandom) throws InvalidAlgorithmParameterException {
        ...
        if (iv != null || iEVP_AEAD_nonce_length == 0) {
            ...
            if (iv != null && iv.length != iEVP_AEAD_nonce_length) {
                StringBuilder sbM5343j = AbstractC1308pu.m5343j(iEVP_AEAD_nonce_length, "Expected IV length of ", " but was ");
                sbM5343j.append(iv.length);
                throw new InvalidAlgorithmParameterException(sbM5343j.toString());
            }
        } else {
            if (!zIsEncrypting) {
    ...
    public int updateInternal(byte[] bArr, int i, int i2, byte[] bArr2, int i3, int i4) {
        checkInitialization();
        if (this.buf == null) {
            f40.m2719o("Cipher not initialized");
            return 0;
        }
    ...
    public int doFinalInternal(ByteBuffer byteBuffer, ByteBuffer byteBuffer2) throws BadPaddingException {
        ...
        try {
            ...
            long j = this.evpAead;
            if (zIsEncrypting) {
                iEVP_AEAD_CTX_open_buf = NativeCrypto.EVP_AEAD_CTX_seal_buf(j, this.encodedKey, this.tagLengthInBytes, byteBuffer2, this.f8444iv, byteBuffer, this.aad);
            } else {
                iEVP_AEAD_CTX_open_buf = NativeCrypto.EVP_AEAD_CTX_open_buf(j, this.encodedKey, this.tagLengthInBytes, byteBuffer2, this.f8444iv, byteBuffer, this.aad);
            }
            if (isEncrypting()) {
                this.mustInitialize = true;
            }
