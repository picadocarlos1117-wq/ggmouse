...
public final class FpsLowSpeedGain {
    public static final FpsLowSpeedGain INSTANCE = new FpsLowSpeedGain();

    private FpsLowSpeedGain() {
    }
    ...
    private final float aplicarEixo(float f, float f2, float f3) {
        ...
        float fM5139E = p32.m5139E(f3, 0.1f, 1.0f);
        if (f2 <= TouchPipeline.SIZE || fM5139E >= 0.999f) {
            return f;
        }
        float fAbs = Math.abs(f);
        return (fAbs <= TouchPipeline.SIZE || fAbs >= f2) ? f : f * fM5139E;
    }
