...
final class CryptoUpcalls {
    ...
    private static byte[] rsaOpWithPrivateKey(PrivateKey privateKey, int i, int i2, byte[] bArr) {
        ...
        if (cipher == null) {
            ArrayList<Provider> externalProviders = getExternalProviders("Cipher.".concat(strConcat));
            int size = externalProviders.size();
            int i3 = 0;
            while (i3 < size) {
                Provider provider = externalProviders.get(i3);
                i3++;
                try {
                    cipher = Cipher.getInstance(strConcat, provider);
                    cipher.init(i2, privateKey);
                    break;
    ...
    private static byte[] signDigestWithPrivateKey(PrivateKey privateKey, byte[] bArr, String str) {
        ...
        if (signature == null) {
            ArrayList<Provider> externalProviders = getExternalProviders(AbstractC1308pu.m5339f("Signature.", str));
            int size = externalProviders.size();
            int i = 0;
            RuntimeException runtimeException = null;
            while (i < size) {
                Provider provider = externalProviders.get(i);
                i++;
                try {
