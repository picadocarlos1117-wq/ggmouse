...
abstract class NativeSslSession {
    ...
    private static void checkRemaining(ByteBuffer byteBuffer, int i) throws IOException {
        if (i < 0) {
            ca0.m1178j(jd0.m3609g(i, "Length is negative: "));
        } else {
            if (i <= byteBuffer.remaining()) {
                return;
            }
            StringBuilder sbM5343j = AbstractC1308pu.m5343j(i, "Length of blob is longer than available: ", " > ");
            sbM5343j.append(byteBuffer.remaining());
